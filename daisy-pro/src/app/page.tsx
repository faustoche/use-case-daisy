...
export default function Home() {
	...
	// Counting number of workshops
	const workshopNumber = WorkshopMock.length;

	// Counting number of participants
	const totalParticipants = WorkshopMock.reduce((total, workshop) => {
		return total + workshop.participants.length;
	}, 0);
		...
		<section className="px-4 py-4">
			...
			<div className="flex flex-row w-full gap-3">
				<div className="flex-1 rounded-xl shadow-sm border border-gray-100 bg-white p-4 text-center border-t-4 border-t-daisy-primary">
					<p className="text-2xl font-bold text-daisy-primary">{workshopNumber}</p>
				...
				<div className="flex-1 rounded-xl shadow-sm border border-gray-100 bg-white p-4 text-center border-t-4 border-t-daisy-accent">
					<p className="text-2xl font-bold text-daisy-accent">{totalParticipants}</p>
