	...
	<main className="flex flex-col min-h-screen">
	  ...
	  <section className="px-4 py-4 flex flex-col gap-2 bg-white flex-1">
		{data.participants.map((participant, index) => {
		  const isPresent = present.has(participant.id);
		  const accent = index % 2 === 0 ? "border-l-daisy-primary" : "border-l-daisy-accent";

		  return (
			<button key={participant.id} onClick={() => toggle(participant.id)} className={`w-full flex items-center justify-between p-4 rounded-xl border border-gray-100 border-l-4 ${accent} shadow-sm text-left transition-colors duration-150 ${
				isPresent ? "bg-purple-50" : "bg-white"
			  }`}>
			  <div className="flex items-center gap-3">


				<div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 transition-colors duration-150 ${
				  isPresent ? "bg-daisy-primary text-white" : "bg-gray-100 text-gray-500"
				}`}>
			  ...
			  <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all duration-150 ${
				isPresent
				  ? "bg-daisy-primary border-daisy-primary"
